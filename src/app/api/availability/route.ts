...
import { addAvailability, listAvailability, removeAvailability } from "@/lib/demo-store";
import { jsonError } from "@/lib/http";

export async function GET() {
  return Response.json({ availability: listAvailability() });
}
...
export async function POST(request: Request) {
  ...
  const slot = addAvailability(parsed.data.startsAt, parsed.data.durationMins);
...
export async function DELETE(request: Request) {
  ...
  if (!id || !removeAvailability(id)) return jsonError("Slot cannot be removed after it has been booked.", 409);
