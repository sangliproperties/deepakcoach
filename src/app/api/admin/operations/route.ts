...
export async function GET() {
  ...
  return Response.json({
    ...
    paymentEvents: await listPaymentWebhookEvents(),
...
export async function POST(request: Request) {
  ...
  if (!parsed.success)
    return jsonError("Please provide valid content details.");
  const data = parsed.data;
  const admin = await requireAdmin();
  if (!admin) return jsonError("Administrator access required.", 403);
  if (
    data.contentType === "youtube-session" &&
    data.title &&
    data.description &&
    data.category &&
    data.duration
    ...
    return Response.json(
      {
        youtubeSession: addYoutubeSession(admin.id, {
          title: data.title,
  ...
  if (
    ...
    return Response.json(
      {
        testimonial: addTestimonial(admin.id, {
          quote: data.quote,
          name: data.name,
          detail: data.detail,
          status: "DRAFT",
  ...
  if (
    ...
    const program = await addProgram(admin.id, {
      slug: data.slug,
      title: data.title,
    ...
    if (!program)
      return jsonError("A program with that slug already exists.", 409);
...
export async function PATCH(request: Request) {
  ...
  if (
    ...
    return Response.json({
      review: await moderateReview(data.id, data.status),
    });
  if (
    ...
    return Response.json({
      testimonial: moderateTestimonial(
        admin.id,
        data.id,
        data.status as "DRAFT" | "PUBLISHED" | "ARCHIVED",
      ),
    });
  if (data.action === "user-role" && data.id && data.role)
    return Response.json({ user: await updateUserRole(data.id, data.role) });
  if (data.action === "program" && data.id && typeof data.active === "boolean")
    return Response.json({
      program: await setProgramActive(data.id, data.active),
    });
  if (data.action === "payment-status" && data.id && data.paymentStatus) {
    if (data.paymentStatus === "PAID")
      return jsonError(
        "Paid status can only be established by a verified provider webhook.",
        403,
      );
    return Response.json({
      payment: await markPayment(data.id, data.paymentStatus),
    });
  }
  if (data.action === "policy")
    return Response.json({
      cancellationPolicy: await updateCancellationPolicy({
...
export async function PUT(request: Request) {
  ...
  if (
    ...
    return Response.json({
      youtubeSession: updateYoutubeSession(admin.id, data.id, {
        title: data.title,
        description: data.description,
        category: data.category,
        duration: data.duration,
  ...
  if (
    ...
    return Response.json({
      testimonial: updateTestimonial(admin.id, data.id, {
        quote: data.quote,
        name: data.name,
        detail: data.detail,
        status: data.status,
  ...
  if (
    ...
    const program = await updateProgram(admin.id, data.id, {
      slug: data.slug,
      title: data.title,
      tagline: data.tagline,
      description: data.description,
    ...
    if (!program) {
      return jsonError(
        "Program was not found or another program already uses this slug.",
        409,
      );
    }
...
export async function DELETE(request: Request) {
  ...
  if (!admin) return jsonError("Administrator access required.", 403);
  const parsed = z
    .object({ contentType: z.enum(["youtube-session"]), id: z.string().min(1) })
    .safeParse(await request.json());
  if (!parsed.success) return jsonError("The requested deletion is invalid.");
  if (!removeYoutubeSession(admin.id, parsed.data.id))
    return jsonError("Content was not found.", 404);
